...
{
	return _diskstream && _session.writable() && (_freeze_record.state != Frozen);
}

bool
Track::can_be_record_enabled ()
{
	return _diskstream && !_diskstream->record_safe() && _session.writable() && (_freeze_record.state != Frozen);
}
